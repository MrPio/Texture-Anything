        ...
        try:
            try:
                pipe = StableDiffusionXLPipeline.from_pretrained(
                    name_or_path, torch_dtype=model_dtype, variant=variant, tokenizer=None, cache_dir="/home/vmorelli-iit.local/Projects/Texture-Anything/.huggingface"
                )
            except EnvironmentError as ex:
                if variant is not None:
                    logger.info("try to load fp32 model")
                    pipe = StableDiffusionXLPipeline.from_pretrained(name_or_path, variant=None, tokenizer=None, cache_dir="/home/vmorelli-iit.local/Projects/Texture-Anything/.huggingface")
                else:
                    raise ex
        except EnvironmentError as ex:
            logger.error(
    ...
    for i, original_path in enumerate(original_paths):
        ...
        if args.tokenizer_cache_dir:
            ...
            if os.path.exists(local_tokenizer_path):
                ...
                tokenizer = CLIPTokenizer.from_pretrained(local_tokenizer_path, cache_dir="/home/vmorelli-iit.local/Projects/Texture-Anything/.huggingface")

        if tokenizer is None:
            tokenizer = CLIPTokenizer.from_pretrained(original_path, cache_dir="/home/vmorelli-iit.local/Projects/Texture-Anything/.huggingface")
