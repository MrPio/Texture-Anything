        ...
        try:
            ...
            for i, info in enumerate(tqdm(image_infos)):
                ...
                if len(batch) > 0 and current_condition != condition:
                    ...
                    batch = []
                if (
                    condition != current_condition and HIGH_VRAM
                ):  # even with high VRAM, if shape is changed
        ...
        for i, info in enumerate(tqdm(image_infos)):
            ...
            if caching_strategy.cache_to_disk:
                ...
                info.text_encoder_outputs_npz = (
                    te_out_npz  # set npz filename regardless of cache availability
                )
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if tokenization_required:
                ...
                input_ids = [
                    ids[0] for ids in self.tokenize_strategy.tokenize(caption)
            ...
            if use_cached_info_for_subset:
                ...
                if strategy is not None:
                    ...
                    for i, img_path in enumerate(tqdm(img_paths)):
                        ...
                        found = False
                        while npz_path_index < len(
                            npz_paths
                        ):  # until found or end of npz_paths
        ...
        for subset in subsets:
            ...
            if os.path.exists(subset.metadata_file):
                if subset.metadata_file.endswith(".jsonl"):
                    logger.info(
                        f"loading existing JSOL metadata: {subset.metadata_file}"
                    )
            ...
            for image_dir in image_dirs:
                npz_paths.extend(
                    glob.glob(os.path.join(image_dir, "*" + strategy.cache_suffix))
                )
            npz_paths = sorted(
                npz_paths, key=lambda item: len(os.path.basename(item)), reverse=True
            )  # longer paths first

            # Match image filename longer to shorter because some images share same prefix
            image_keys_sorted_by_length_desc = sorted(
                metadata.keys(), key=len, reverse=True
            )
            ...
            for image_key in image_keys_sorted_by_length_desc:
                ...
                if image_size is None:
                    ...
                    if npz_path is not None:
                        npz_paths.remove(
                            npz_path
                        )  # remove to avoid matching same file (share prefix)
                        abs_path = npz_path
            ...
            if size_set_from_metadata > 0:
                logger.info(
                    f"set image size from metadata: {size_set_from_metadata}/{len(image_keys_sorted_by_length_desc)}"
                )
...
def cache_batch_text_encoder_outputs_sd3(
    ...
    for info, lg_out, t5_out, pool in zip(image_infos, b_lg_out, b_t5_out, b_pool):
        # debug: NaN check
        if (
            torch.isnan(lg_out).any()
            or torch.isnan(t5_out).any()
            or torch.isnan(pool).any()
        ):
            raise RuntimeError(
                f"NaN detected in text encoder outputs: {info.absolute_path}"
            )

        if cache_to_disk:
            save_text_encoder_outputs_to_disk(
                info.text_encoder_outputs_npz, lg_out, t5_out, pool
            )
        else:
    ...
    kwargs_handlers = [
        (
            InitProcessGroupKwargs(
                ...
                timeout=(
                    datetime.timedelta(minutes=args.ddp_timeout)
                    if args.ddp_timeout
                    else None
                ),
            )
...
def sample_images_common(
    ...
    default_scheduler = get_my_scheduler(
        sample_sampler=args.sample_sampler, v_parameterization=args.v_parameterization
    )
