    ...
    for validation_prompt, validation_image in zip(
        ...
        try:
            interpolation = getattr(
                transforms.InterpolationMode, args.image_interpolation_mode.upper()
            )
...
def prepare_train_dataset(dataset, accelerator):
    ...
    def preprocess_train(examples):
        ...
        if args.invert_conditioning_image:
            conditioning_images = [
                PIL.ImageOps.invert(image) for image in conditioning_images
            ]
        conditioning_images = [
...
def collate_fn(examples):
    ...
    mask_values = torch.stack(
        [
            F.interpolate(
                example["mask_values"].float().unsqueeze(0).unsqueeze(0),
